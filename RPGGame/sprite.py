...
class Player(pygame.sprite.Sprite ):
    def __init__(self, game, x, y):
        ...
        # # self.image.fill(RED)
        # this is one way to do it but it would take a while to create every animation for the character to move so we have a different solution than just to make tons of the same code over and over again

        self.rect = self.image.get_rect()
        self.rect.x = self.x
        self.rect.y = self.y
        #every sprite in pygame it has an image which is what it looks like and a rect, a rect is basically where its position or how big it is, functions similiarly like hitboxes
    ...
    def movement(self): #the codes below that i commented is a camera system, but it's more like it moves every sprite on the map but i don't really like the result so i keep it commented.
        ...
        if keys[pygame.K_DOWN]:
            ...
            self.y_change += PLAYER_SPEED
            self.facing = 'down'
        #in pygame the y axis starts at 0 at Up but when it goes down it basically adds up to the qeuation therefore it is listed as += for the Down variable
    ...
    def collide_blocks(self, direction): #the codes below is also part of the "camera" system on above but it's to fix the bug where the camera keeps moving even though the player is already on the wall, thus making the camera runs away to the void.
        if direction == "x":
...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self): #this is where i call the functions on this respective class
        self.movement()
        self.animate()
        self.rect.x += self.x_change
        self.rect.y += self.y_change
    ...
    def movement(self): # this is for the enemy to move right and left 
        if self.facing == 'left':
    ...
    def collide_blocks(self, direction): #this codes below is for the collision of blocks for the enemy sprite
        if direction == "x":
            hits = pygame.sprite.spritecollide(self, self.game.blocks, False)
            if hits:
                if self.x_change > 0:
...
class Block(pygame.sprite.Sprite): 
    def __init__(self, game, x, y):
        ...
        self.image = self.game.terrain_spritesheet.get_sprite(960, 545, self.width, self.height)
        # self.image = pygame.Surface([self.width, self.height])
...
class Button:
    def __init__(self, x, y, width, height, fg, bg, content, fontsize):
        self.font = pygame.font.Font('STENCIL.ttf', fontsize)
...
class Attack(pygame.sprite.Sprite):
    ...
    def collide(self):
        ...
        #if its written as False the enemy wont be killed, but in True the enemy will be killed or disappear
